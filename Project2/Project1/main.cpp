...
int gra() {
	...
	//sf::Music music;
	//music.openFromFile("C:/Projects/jojo.wav");

	sf::Texture player;
	sf::RectangleShape gracz;
	sf::RectangleShape pocisk;
	sf::RectangleShape brando;
	pocisk.setPosition(100000, 10000000);
	sf::Texture enemy;
	sf::Texture arena;
	sf::Texture lit;
	sf::Texture dio;
	dio.loadFromFile("C:/Projects/world.png");
	brando.setSize(sf::Vector2f(width, height));
	brando.setTexture(&dio);
	sf::Texture end;
	sf::Text czas;
	sf::Text tabela;
	sf::Font font;
	...
	sf::Text zaczas;
	zaczas.setFont(font);
	zaczas.setCharacterSize(40);
	zaczas.setString("Nacisnij spacje zeby zatrzymac czas");
	zaczas.setPosition(width / 2 - 300, height / 2 + 100);
	window.display();
	...
	while (window.isOpen())
	{
		...
		sf::Time secs2 = zawarudocooldown.getElapsedTime();
		sf::Time secs = zawarda.getElapsedTime();
		if (secs.asSeconds()>0.5 && secs2.asSeconds()>10) {
			if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space))
			{
				if (zawardo == 0) {
					...
					zawardo = 0;
					zawarda.restart();
					zawarudocooldown.restart();
				}
			}

		}
		secs = zawarda.getElapsedTime();
		if (secs.asSeconds() > 5 && zawardo == 1) {
			zawardo = 0;
			zawarda.restart();
			zawarudocooldown.restart();
		}
		...
		if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
			...
			if (czasz>0.1) {
				...
				pociski[click].setPosition(width / 2, height / 2);//1
				wlasciwoscipociskow.push_back(sf::Vector2f(predkoscpociskux, predkoscpociskuy));
				if (click > 40) {
					wlasciwoscipociskow.erase(wlasciwoscipociskow.begin());
		...
		if (playerflag == 1) {
			...
			for (a = 0; a < przeciwnicy.size(); a++) {
				...
				przeciwnicy[a].setPosition(przeciwnicy[a].getPosition().x + globalx, przeciwnicy[a].getPosition().y + globaly);

				if (zawardo == 0) {
					przeciwnicy[a].setPosition(przeciwnicy[a].getPosition().x + przeciwnikx, przeciwnicy[a].getPosition().y + przeciwniky);
				}
			...
			for (a = 0; a < pociski.size(); a++) {
				pociski[a].setPosition(pociski[a].getPosition().x + globalx, pociski[a].getPosition().y + globaly);
				if (zawardo == 0) {
					pociski[a].setPosition(pociski[a].getPosition().x + wlasciwoscipociskow[a].x, pociski[a].getPosition().y + wlasciwoscipociskow[a].y);
				}
			...
			for (a = 0; a < pociski.size(); a++) {
				for (b = 0; b < przeciwnicy.size(); b++) {
					if (przeciwnicy[b].getPosition().x<pociski[a].getPosition().x + 40 && przeciwnicy[b].getPosition().x>pociski[a].getPosition().x - 40 && przeciwnicy[b].getPosition().y<pociski[a].getPosition().y + 40 && przeciwnicy[b].getPosition().y>pociski[a].getPosition().y - 40) {
						int randy = rand() % 4;
						if (randy == 0) {
							//float randomx = rand() % width * 4 - width * 2;
							float randomy = rand() % height * 4 - width * 2;
			...
			if (secs2.asSeconds() > 10.0 && zawardo == 0) {
				window.draw(zaczas);
			}
			////////////
		}
			...
			if (licznik < 74) {
				...
				if (sf::Keyboard::isKeyPressed(sf::Keyboard::W))
				{
					tabela.setPosition(tabela.getPosition().x, tabela.getPosition().y + 7);
					if (tabela.getPosition().y > 10) {
						tabela.setPosition(tabela.getPosition().x, 10);
					}
				}
				if (sf::Keyboard::isKeyPressed(sf::Keyboard::S))
				{
